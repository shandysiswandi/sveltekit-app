<!-- source: https://github.com/afsar-dev/Nurui/blob/main/src/components/nurui/splash-cursor.tsx -->

<script lang="ts">
  // Svelte 5 Runes version of the fluid splash cursor effect.
  ...
  // Use the $props rune to get the component's properties with default values.
  let {
    SIM_RESOLUTION = 128,
    DYE_RESOLUTION = 1440,
    ...
    TRANSPARENT = true,
  }: SplashCursorProps = $props();

  // This will hold the reference to the canvas element.
  let canvasEl: HTMLCanvasElement;

  // The $effect rune runs after the component has mounted, similar to React's useEffect.
  // It's the perfect place for WebGL initialization and event listeners.
  $effect(() => {
    const canvas = canvasEl;
    if (!canvas) return;

    // --- The entire WebGL and event listener logic from the React component goes here ---
    // --- It is copied verbatim as it's standard WebGL/JavaScript code. ---
    ...
    // The return function from $effect serves as the cleanup, similar to useEffect's return.
    return () => {
...
<div class="pointer-events-none fixed top-0 left-0 z-50 h-full w-full">
  <canvas bind:this={canvasEl} id="fluid" class="block h-screen w-screen"></canvas>
</div>
